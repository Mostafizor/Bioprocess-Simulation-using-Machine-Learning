...
from test_4_datasets import test
...
while avg_mse > 0.9:
    ...
    training_inputs = np.split(training_inputs, 404)
    training_labels = np.split(training_labels, 404)
...
online.to_excel('Data3/Optimised_Networks/online_manual {x}_{y}_{a}_{b}_{c}.xlsx'.format(x=HL, y=HN, a=EPOCHS, b=LR, c=BATCH_SIZE))
offline.to_excel('Data3/Optimised_Networks/offline_manual {x}_{y}_{a}_{b}_{c}.xlsx'.format(x=HL, y=HN, a=EPOCHS, b=LR, c=BATCH_SIZE))
avg_mse.to_excel('Data3/Optimised_Networks/avg_mse_manual {x}_{y}_{a}_{b}_{c}.xlsx'.format(x=HL, y=HN, a=EPOCHS, b=LR, c=BATCH_SIZE))

torch.save(rnn.state_dict(), 'Data3/Optimised_Networks/Models/manual {x}_{y}_{a}_{b}_{c}.pt'.format(x=HL, y=HN, z=HN, a=EPOCHS, b=LR, c=BATCH_SIZE))