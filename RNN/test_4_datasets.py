...
def test(test_inputs, test_labels, net, BATCH_SIZE):
    ...
    LI1, LI2, LI3, LI4 = test_X[0][0][3], test_X[1][0][3], test_X[2][0][3], test_X[3][0][3]
    NIC1, NIC2, NIC3, NIC4 = test_X[0][0][4], test_X[1][0][4], test_X[2][0][4], test_X[3][0][4]
    predictions_online = []
    for index1, element in enumerate(test_X):
        ...
        if index1 == 2:
            for index2, row in enumerate(element):
                ...
                predictions_online.append([BC, NC, LP, LI3, NIC3])
        ...
        if index1 == 3:
            for index2, row in enumerate(element):
                ...
                predictions_online.append([BC, NC, LP, LI4, NIC4])
    predictions_online = np.array(predictions_online)

    predictions_offline = []
    BC1, BC2, BC3, BC4 = test_X[0][0][0], test_X[1][0][0], test_X[2][0][0], test_X[3][0][0]
    NC1, NC2, NC3, NC4 = test_X[0][0][1], test_X[1][0][1], test_X[2][0][1], test_X[3][0][1]
    LP1, LP2, LP3, LP4 = test_X[0][0][2], test_X[1][0][2], test_X[2][0][2], test_X[3][0][2]
    ...
    for index1, element in enumerate(test_X):
        ...
        for index2, row in enumerate(element):
            ...
            if index1 == 2:
                net_out, hidden = net(Variable(torch.Tensor([    
                    [[BC3, NC3, LP3, LI3, NIC3]]
                ])), hidden)
                BC = BC3 + net_out[0][0][0]
                NC = NC3 + net_out[0][0][1]
                LP = LP3 + net_out[0][0][2]
                predictions_offline.append([float(BC), float(NC), float(LP), float(LI3), float(NIC3)])
                BC3 = BC
                NC3 = NC
                LP3 = LP
            if index1 == 3:
                net_out, hidden = net(Variable(torch.Tensor([    
                    [[BC4, NC4, LP4, LI4, NIC4]]
                ])), hidden)
                BC = BC4 + net_out[0][0][0]
                NC = NC4 + net_out[0][0][1]
                LP = LP4 + net_out[0][0][2]
                predictions_offline.append([float(BC), float(NC), float(LP), float(LI4), float(NIC4)])
                BC4 = BC
                NC4 = NC
                LP4 = LP
