...
class Net(nn.Module):
	...
	def __init__(self, HN):	
		...
		self.fc2 = nn.Linear(self.HN, self.HN)                                                   # Hidden layer 2: each node takes in 15 values, contains 15 nodes hence outputs 15 values.
		self.fc3 = nn.Linear(self.HN, 3)                                                    # Output Layer: each node takes in 15 values, contain 3 nodes (one for each rate of change: X, N and Lu) hence outputs 3 values.
